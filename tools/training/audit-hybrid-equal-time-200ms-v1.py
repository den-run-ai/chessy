#!/usr/bin/env python3
"""Independent python-chess replay of the canonical200ms diagnostic archive.
...
def paired_stats(pairs):
    # Match the already reviewed JS small-df table:20clusters =>df19,t=1.729.
    require(len(pairs)==20,'exact20opening pairs required')
    mean=statistics.mean(pairs);sd=statistics.stdev(pairs)
    return mean,sd,max(0,mean-1.729*sd/math.sqrt(20))
...
def timing(records,time_ms):
    ...
    for side in ('baseline','candidate'):
        ...
        result[side]={'moves':len(rows),'totalElapsedMs':total,'totalNodes':nodes,'nodesPerSecond':1000*nodes/total,
            ...
            'medianObservedMs':percentile(elapsed,.5),'p95ObservedMs':percentile(elapsed,.95),'maximumObservedMs':max(elapsed),'overTwoTimesRequested':sum(t>2*time_ms for t in elapsed),
...
def audit_game(rows, task, modules):
    ...
    for ply, record in enumerate(searched):
        ...
        if record["result"]["stopReason"]=="time-limit":require(record["elapsedMs"]>=task["timeMs"],"time-limit stopped before requested budget")
...
def audit(repo,registration,digest,run,output):
    ...
    source=original['openings'];require(len(source)==100 and [o['index'] for o in source]==list(range(100)),'unchanged100opening source required')
    selected=sorted(source,key=lambda o:hashlib.sha256((r['protocol']['openingSelection']['salt']+'|'+str(o['index'])).encode()).hexdigest())[:20]
    require(old.equal(r['openings'],selected),'hash-selected opening bank differs')
    require([o['index'] for o in selected]==r['protocol']['openingSelection']['selectedOriginalIndices'],'frozen selected indices differ')
    precursor=r['precursor'];prior_summary=inputs.json(precursor['summary']['path'],precursor['summary']);prior_audit=inputs.json(precursor['independentAudit']['path'],precursor['independentAudit'])
    expected_prior=r['protocol']['precursor']
    require(precursor['summary']['sha256']==expected_prior['summarySha256'] and precursor['independentAudit']['sha256']==expected_prior['independentAuditSha256'],'precursor byte binding differs')
    require(prior_summary['status']=='completed' and prior_summary['completedGames']==prior_summary['plannedGames']==200 and prior_summary['byArm']['hybrid']['candidateScore']==.5,'completed fixed-node precursor differs')
    require(prior_audit['status']=='PASS' and prior_audit['games']==200 and prior_audit['mismatches']==0 and prior_audit['summarySha256']==precursor['summary']['sha256'],'precursor independent audit differs')
    require(prior_summary['registrationSha256']==prior_audit['registrationSha256']==expected_prior['registrationSha256'],'precursor registration differs')
    for p in (HERE,HERE.with_name('audit-natural-runtime.py')):inputs.read(p,{'sha256':old.digest(p.read_bytes())})
    protocol=old.strict_json((repo/'eval/training/hybrid-equal-time-200ms-v1.json').read_bytes());require(old.equal(protocol,r['protocol']),'registered protocol changed')
    require(r['originalRegistration']['sha256']==protocol['originalRegistrationSha256'],'original identity differs')
    require(protocol['totalGames']==40 and protocol['timeMs']==200 and protocol['nodeLimit']==0 and protocol['maxSearchedPlies']==180 and protocol['workloadWatchdogMs']==2100000 and protocol['arms']==['hybrid'] and len(r['openings'])==20,'protocol dimensions differ')
    expected=[]
    for opening in r['openings']:
        index=opening['index'];colors=['b','w'] if index%2 else ['w','b']
        for color in colors:expected.append({'taskId':f'hybrid-t200-o{index}-{color}','arm':'hybrid','opening':opening,'candidateColor':color,'timeMs':200,'nodeLimit':0})
    ...
    require(summary['schema']=='chessy.hybrid-equal-time-200ms-diagnostic-summary.v1' and summary['status']=='completed' and summary['failure'] is None and summary['completedGames']==summary['plannedGames']==40,'full completed batch required before raw score audit')
    ...
    require(manifest['schema']=='chessy.hybrid-equal-time-200ms-diagnostic-raw-manifest.v1' and manifest['status']=='completed' and manifest['failure'] is None and manifest['registration']['sha256']==digest,'raw manifest scope/binding differs')
    ...
    require(len(archived['entries'])==archive_info['entries']==len(expected_receipts)==41,'canonical archive inventory differs')
    members={}
    ...
    for index,(task,item) in enumerate(zip(expected,manifest['games'])):
        ...
        flattened={'taskId':task['taskId'],'openingIndex':task['opening']['index'],'openingName':task['opening']['name'],
            'prefixUci':task['opening']['prefixUci'],'timeMs':task['timeMs'],'candidateColor':task['candidateColor'],'maxPlies':180,'maxDepth':30,'nodeLimit':0}
        ...
        moves+=len(rows)-2
        if (index+1)%20==0:print(json.dumps({'independentlyReplayedGames':index+1,'moves':moves}),flush=True)
    by_arm={}
    for arm in protocol['arms']:
        ...
        mean,sd,lower=paired_stats(pairs);times=timing(records[arm],200)
        by_arm[arm]={'games':len(group),'openingPairs':20,'wins':sum(game['candidateScore']==1 for game in group),'draws':sum(game['candidateScore']==.5 for game in group),
    ...
    inputs.recheck()
    result={'schema':'chessy.hybrid-equal-time-200ms-independent-audit.v1','status':'PASS','registrationSha256':digest,'summarySha256':old.digest(summary_raw),
        ...
        'mismatches':0,'canonicalArchiveMembers':41,'rawGameSource':'authenticated retained gzip archive only','mutableLiveGameFilesRead':0,'byArm':by_arm,'pythonChessVersion':old.chess.__version__,'engineSearchesRunByAuditor':0,'inputs':{str(p):info for p,info in inputs.files.items()},
        'limitations':['Independent legality/repetition/adjudication uses python-chess on authenticated retained archive members; mutable live game files are never used and wall-clock numbers are not remeasured.',
            'Unchanged hybrid versus shipped HCE at200ms; expanded HCE remains an offline control, not an additional match arm.',
            'The20-pair exposed subset and200ms budget are developmental; descriptive clustered bounds do not establish Elo or shipping acceptance.']}
