#!/usr/bin/env python3
"""Bounded v3 phase/capacity/schedule ablation. Private checkpoints; no engine integration.
...
import h4_v2_model as single_model
import h4_v3_model as phase_model

ROOT = Path(__file__).resolve().parents[2]
CONTRACT = ROOT / "eval/training/natural-nnue-h4-v3.json"
CONTRACT_SHA = "89321a8e3f79b910b30fdeddd872bb3228d1886db471b6d654f59602943a0950"
...
def implementation():
    ...
    paths = [Path(__file__), CONTRACT, ROOT / "tools/training/h4_v2_model.py", ROOT / "tools/training/h4_v3_model.py",
             ROOT / "test/training/h4-v3-reference.js",
...
def backend(config):
    return single_model if config["mode"] == "residual-shipped-hce" else phase_model


def baseline(data, config):
    return data.baseline_cp


def extras(config, data, indexes=None):
    if backend(config) is single_model:
        return {}
    phase = phase_model.phase_from_fens(data.fens)
    return {"phase_units": phase if indexes is None else phase[indexes]}


def settings_for(config, settings):
    result = dict(settings)
    result["epochs"] = config["epochs"]
    result["quantizationAwareStartEpoch"] = config["epochs"] * 2 // 3 + 1
    return result


def phase_guard(base, candidate):
    reasons = []
    for phase in util.PHASES:
...
def fit(config, seed, train, validation, settings, deadline, epochs=None):
    ...
    train_extra = extras(config, train)
    val_extra = extras(config, validation) if validation is not None else {}
    weights = np.where(train.phase == "endgame", config["endgameWeight"], 1.).astype(float)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for start in range(0, train.rows, settings["batchSize"]):
            ...
            _, grad, diagnostics = model.loss_and_grad(params, *features, train.target[indexes],
                ...
                **({"phase_units": train_extra["phase_units"][indexes], "sample_weight": weights[indexes]} if train_extra else {}))
            step += 1
        ...
        if epoch % settings["checkpointEveryEpochs"] == 0 or epoch == epochs:
            quantized = model.quantize(params, hidden=hidden, baseline_id="shipped-hce")
            train_cp = quantized.predict_cp(*train_features, baseline_cp=offset, **train_extra)
            _, full_grad, components = model.loss_and_grad(params, *train_features, train.target,
                ...
                **({"phase_units": train_extra["phase_units"], "sample_weight": weights} if train_extra else {}))
            ...
            if validation is not None:
                val_cp = quantized.predict_cp(*val_features, baseline_cp=baseline(validation, config), **val_extra)
...
def parity(params, config, data, output):
    model = backend(config)
    extra = extras(config, data)
    quantized = model.quantize(params, hidden=config["hidden"], baseline_id="shipped-hce")
    features, offset = model.features_from_fens(data.fens), baseline(data, config)
    integer = quantized.predict_cp(*features, baseline_cp=offset, **extra)
    floating = model.predict_cp(params, *features, hidden=config["hidden"], baseline_cp=offset, **extra)
    with tempfile.TemporaryDirectory(prefix="v3-parity-", dir=output) as name:
        tmp = Path(name)
        ...
        result = subprocess.run(["node", str(ROOT / ("test/training/h4-v2-reference.js" if model is single_model else "test/training/h4-v3-reference.js")),
...
def choose(records, configurations, seeds, comparators):
    if len(records) != len(configurations)*len(seeds):
    ...
    for config in configurations:
        ...
        for record in found:
            if (not np.isfinite(record["selectedInnerCe"]) or record["epochsRun"] != config["epochs"]
                    or not config["epochs"]*2//3 < record["selectedEpoch"] <= config["epochs"]):
                raise ValueError("invalid development checkpoint")
            if set(record["checks"]) != {"inner-train", "inner-validation", "authored"}:
                raise ValueError("incomplete development checks")
            if any(check["reasons"] or check["integerMismatches"] for check in record["checks"].values()):
                raise ValueError("development numerical gate failure")
            for role in ("inner-train", "inner-validation"):
                quality = record["quality"][role]
                metrics = [quality, *quality["byPhase"].values()]
                if any(not all(np.isfinite(value) for key, value in metric.items() if key != "byPhase")
                       for metric in metrics):
                    raise ValueError("nonfinite development quality")
        ranked = sorted(found, key=lambda r: (r["selectedInnerCe"], r["seed"]))
        median = ranked[1]
        reasons = ["shipped-hce:"+reason for reason in
                   phase_guard(comparators["shipped-hce"], median["quality"]["inner-validation"])]
        if reasons:
...
def ablations(records):
    by_config = {}
    for record in records:
        by_config.setdefault(record["config"]["id"], []).append(record["selectedInnerCe"])
    pairs = [("phase-heads-and-head-regularization", "h4-single-e300", "h4-phase-e300"),
             ("h4-training-budget", "h4-phase-e300", "h4-phase-e900"),
             ("endgame-weight", "h4-phase-e900", "h4-phase-e900-eg2"),
             ("capacity", "h4-phase-e900-eg2", "h8-phase-e900-eg2"),
             ("h8-training-budget", "h8-phase-e900-eg2", "h8-phase-e1800-eg2")]
    result = []
    for name, a, b in pairs:
        if a not in by_config or b not in by_config:
            continue
        left, right = by_config[a], by_config[b]
        improvement = 1-float(np.median(right))/float(np.median(left))
        result.append({"ablation": name, "control": a, "candidate": b,
            "relativeMedianCeImprovement": improvement,
            "controlSeedRange": [min(left), max(left)], "candidateSeedRange": [min(right), max(right)],
            "smallPositiveGainBelowPoint1Percent": 0<improvement<.001,
            "boundedGridOnly": True, "convergenceClaimed": False})
    return result
...
def run(bundle, output):
    ...
    for config in rules["configurations"]:
        for seed in rules["training"]["seeds"]:
            params, record = fit(config, seed, inner_train, inner_validation, settings_for(config, rules["training"]), deadline)
    ...
    decision = choose(records, rules["configurations"], rules["training"]["seeds"], comparators["inner-validation"])
    ...
    if config is None:
        summary = {"schema": "chessy.h4-v3-screen.v1", "head": head, "contractSha256": CONTRACT_SHA,
                   "researchOnly": True, "productionFitAllowed": False, "paidSpendUsd": 0,
                   "decision": decision, "runs": records,
                   "comparators": comparators, "ablations": ablations(records), "testOpened": False,
                   "testEligible": False, "stopReasons": decision["stopReasons"],
                   "trainingSeconds": sum(r["seconds"] for r in records), "runtimeIntegrationPerformed": False,
        ...
        reserve("selection-frozen", {"sha256": digest(encoded(summary)), "testEligible": False}, expected)
        return
    params, final_fit = fit(config, decision["seed"], train, None, settings_for(config, rules["training"]), deadline, epochs=decision["epochs"])
    ...
    summary = {"schema": "chessy.h4-v3-screen.v1", "head": head, "contractSha256": CONTRACT_SHA,
               "researchOnly": True, "productionFitAllowed": False, "paidSpendUsd": 0,
               "innerRows": {"train": inner_train.rows, "validation": inner_validation.rows},
               "decision": decision, "comparators": comparators, "runs": records, "final": final, "ablations": ablations(records),
    ...
    print(json.dumps({"stage": "completed", "output": str(output), "selected": config["id"],
                      "testOpened": False, "testEligible": not stop_reasons, "stopReasons": stop_reasons,
                      "trainingSeconds": summary["trainingSeconds"]}), flush=True)


def test_registry():
    key = digest((SUMMARY_SHA+"\0nnue-test").encode())
    return ROOT / ".research-state" / ("dataset-"+key) / "test-opened.json"


def reserve_test(value, expected):
    # Existing v1/v2 markers used contract-scoped keys. Any such consumed test
    # in this checkout blocks opening: conservative across legacy contracts.
    if any((ROOT / ".research-state").glob("*/test-opened.json")):
        raise FileExistsError("test already opened by current or legacy experiment")
    publish(test_registry(), encoded({**value, "summarySha256": SUMMARY_SHA, "role": "nnue-test"}), expected)


def open_test(bundle, output):
    rules, code_expected, head = implementation()
    auth = data_io.authenticate(bundle)
    expected = {**code_expected, **auth.expected}
    summary_raw = (output / "selection.json").read_bytes()
    summary = json.loads(summary_raw)
    state_raw = (state_root() / "selection-frozen.json").read_bytes()
    frozen = json.loads(state_raw)
    if (digest(summary_raw) != frozen["sha256"] or not frozen["testEligible"]
            or not summary["testEligible"] or summary["stopReasons"]
            or summary["contractSha256"] != CONTRACT_SHA or summary["head"] != head):
        raise ValueError("test requires same clean implementation and eligible frozen selection")
    expected[output / "selection.json"] = digest(summary_raw)
    expected[state_root() / "selection-frozen.json"] = digest(state_raw)
    config = summary["decision"]["config"]
    artifacts = summary["final"]["artifacts"]
    for item in artifacts.values():
        path = output / item["path"]
        if path.parent != output or path.name != item["path"]:
            raise ValueError("checkpoint must be a direct output child")
        expected[path] = item["sha256"]
        util.read_exact(path, item["sha256"])
    params = np.asarray(json.loads(util.read_exact(output / artifacts["float"]["path"], artifacts["float"]["sha256"])))
    quantized = backend(config).quantize(params, hidden=config["hidden"], baseline_id="shipped-hce")
    if quantized.to_bytes() != util.read_exact(output / artifacts["binary"]["path"], artifacts["binary"]["sha256"]):
        raise ValueError("frozen float/integer checkpoint mismatch")
    marker = {"selectionSha256": digest(summary_raw), "binarySha256": artifacts["binary"]["sha256"], "contractSha256": CONTRACT_SHA}
    reserve_test(marker, expected)
    ...
    result = {"schema": "chessy.h4-v3-test.v1", **marker, "testOpened": True,
              "quality": metrics, "floatQuality": util.quality_metrics(test_float,test), "checks": test_check,
              "comparators": {name:util.quality_metrics(b,test) for name,b in bases.items()},
    ...
    print(json.dumps({"stage": "test-completed", "offlinePass": not test_reasons, "stopReasons": test_reasons}), flush=True)
...
def main():
    ...
    if args.open_test_from is not None:
        open_test(args.bundle.absolute(), args.open_test_from.absolute())
        return
