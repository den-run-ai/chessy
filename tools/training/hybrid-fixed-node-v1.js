...
function register(a){
  ...
  const openings=H.openings();same(openings,original.openings,'unchanged exposed openings');
  ...
  const r={schema:'chessy.hybrid-fixed-node-diagnostic-registration.v1',createdAtUtc:new Date().toISOString(),researchOnly:true,formalPass:false,
    shippingOrEloClaimAllowed:false,candidateAdvancementAllowed:false,protocol,modules,openings,tasks,environment,
    originalRegistration:source.identity,implementation:IMPLEMENTATION.map(file=>file===CONTRACT?contract.identity:identity(file)),
    executionRoot:path.dirname(a.output),noRerunLedger:path.join(path.dirname(original.offline.path),'.hybrid-fixed-node-v1.started.json')};
...
async function run(a){
  const r=preflight(a.registration,a['registration-sha256']);check(path.dirname(a.output)===r.executionRoot&&!fs.existsSync(a.output),'new output must use registered fixed-node root');
  ...
  const manifest={schema:'chessy.hybrid-fixed-node-diagnostic-raw-manifest.v1',registration:identity(a.registration),startedAtUtc,completedAtUtc:new Date().toISOString(),
  ...
  const rawManifest=write(path.join(a.output,'raw-manifest.json'),manifest),summary={schema:'chessy.hybrid-fixed-node-diagnostic-summary.v1',status:failure?'failed':'completed',failure,
...
if(require.main===module){if(process.argv[2]==='--internal-child')process.once('message',r=>child(r).catch(error=>{process.send({control:'failed',error:error.message},()=>process.disconnect());process.exitCode=1;}));
    ...
    return audit(preflight(a.registration,a['registration-sha256']),a.output,read(path.join(a.output,'raw-manifest.json')));})().then(value=>{
      console.log(JSON.stringify(value));if(value.status==='failed')process.exitCode=1;}).catch(error=>{console.error('hybrid-fixed-node-v1: '+error.message);process.exitCode=1;});}
module.exports={register,preflight,audit,liveDiscrepancies,args,awaitCaptured,makeTasks,playGame,auditGame,analyzeAudited,timedSearch};
