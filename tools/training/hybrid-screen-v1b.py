...
util=shared.util
CONTRACT=ROOT/'eval/training/hybrid-v1b.json'
...
def implementation():
    ...
    paths=[CONTRACT,Path(__file__),ROOT/'tools/training/hybrid-model-v1.py',ROOT/'tools/training/hybrid-reference-v1.js',
           ROOT/'tools/training/hybrid-selftest-v1.py',ROOT/'tools/training/hybrid-selftest-v1b.py',ROOT/'eval/training/hybrid-v1.json',ROOT/'tools/training/natural-nnue-h4-v3.py',
...
def integer_comparators(bases,rows):
    result={}
    for name,value in bases.items():
        cp=np.asarray(value)
        if cp.shape!=(rows,) or not np.all(np.isfinite(cp)) or np.any(cp!=np.trunc(cp)) or np.any(np.abs(cp)>10000):
            raise ValueError('comparator must contain exact bounded integral cp')
        result[name]=cp.astype(np.int64)
    return result
...
def run(bundle,checkpoints,output):
    ...
    bases=integer_comparators(util.comparator_predictions(auth,inner),inner.rows)
    ...
    summary={'schema':'chessy.hybrid-screen.v1b','head':head,'contractSha256':CONTRACT_SHA,'researchOnly':True,'productionFitAllowed':False,
             ...
             'dataCaveat':rules['data']['caveat'],'recovery':rules['recovery'],'stopReasons':list(failures),'testEligible':False,'inputSha256':{str(p):d for p,d in expected.items()}}
    if winner:
        ...
        outer=data_io.load_role(auth,'nnue-validation');outer_bases=integer_comparators(util.comparator_predictions(auth,outer),outer.rows)
