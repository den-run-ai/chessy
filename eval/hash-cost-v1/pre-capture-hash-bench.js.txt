...
const positions=()=>Bench.POSITIONS.map(([name,fen],i)=>({name,fen,phase:i<2?'opening':i>=8&&i<16?'endgame':'middlegame'}));
const warmup={nodeLimit:10000,maxDepth:111,timeMs:0,quiesce:true,rounds:2};
const dependencies=()=>[__filename,path.join(ROOT,'tools/search/hash-cost.js'),path.join(ROOT,'experiments/wasm/bench.js'),path.join(ROOT,'test/fixtures/wasm-r69-signatures.json')].map(p=>({path:path.relative(ROOT,p),sha256:sha(fs.readFileSync(p))}));
...
function register(file,mode,modules){
  ...
  check(modules.length===2,'exactly one reference and one frozen candidate required');
  const plan={schema:'chessy.searched-board-hash.benchmark.v1',mode,created:new Date().toISOString(),researchOnly:true,
    productionIntegrationAllowed:false,strengthClaimAllowed:false,selectionAllowed:mode==='fixed',
    positions:positions(),options:options(mode),warmup,runtime:runtime(),
...
async function run(file,out){
  ...
  check(plan.schema==='chessy.searched-board-hash.benchmark.v1'&&plan.researchOnly===true&&plan.productionIntegrationAllowed===false&&plan.strengthClaimAllowed===false,'invalid plan');
