/*
 * Browser probe worker — runs the JS engine and the Zig/WASM module through
 * the same differential protocol as experiments/wasm/bench.js, inside a real
 * Web Worker (the production execution context for both engines).
 *
...
async function runProbe(config) {
  ...
  importScripts(config.assetsBase + '/experiments/wasm/js/wasm-engine.js');
  ...
  const wasm = await ChessyWasm.load(bytes);
