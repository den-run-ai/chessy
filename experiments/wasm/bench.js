/*
 * Chessy Zig/WASM feasibility benchmark.
 *
...
async function loadWasmEngine(wasmPath) {
...
function signatureDifferences(candidate, baseline) {
  return SIGNATURE_FIELDS.filter(function (field) {
    ...
    return field + ': wasm=' + candidate[field] + ', js=' + baseline[field];
  });
}
...
function benchmarkPosition(wasm, js, fen, index, options, reps, minimumMs) {
  ...
  return {
    wasm: summarize(wasmSamples, 'WASM'),
    js: summarize(jsSamples, 'JavaScript'),
...
async function main(argv) {
  ...
  const wasm = await loadWasmEngine(config.wasmPath);
  const js = loadJsEngine();
  const options = {
  ...
  for (let index = 0; index < POSITIONS.length; index++) {
    ...
    console.log(
      ...
      '  WASM ' + pair.wasm.ms.toFixed(2).padStart(8) + ' ms' +
      '  JS ' + pair.js.ms.toFixed(2).padStart(8) + ' ms' +
  ...
  const outcome = gateOutcome(geomeanRatio, familyRatios);
