...
const Q1 = 16384n, Q2 = 4096n, CP_SCALE = 400n;
...
function expectedMetadata(hidden, mode) {
  return {
    schema: 'chessy-h4-v2-quantized-v1', researchOnly: true,
    inputs: 768, hidden, parameters: 771 * hidden + 1,
    parameterBytes: 1544 * hidden + 4,
    q1: Number(Q1), q2: Number(Q2), cpScale: Number(CP_SCALE), maximumPieces: 32,
    layout: `W1[768,${hidden}]:i16,b1[${hidden}]:i32,W2[${2 * hidden}]:i16,b2[1]:i32;little-endian`,
    featureOrder: 'PNBRQKpnbrqk; a8=0; black: color-swap,square^56',
    outputOrder: `side-to-move[${hidden}],other[${hidden}]`,
    ...
    mode, fixedBaseline: mode === 'net-only' ? 'none' : 'shipped-hce',
...
function loadModel(bytes, metadata) {
  ...
  const hidden = metadata.hidden, mode = metadata.mode;
  if (hidden !== 4 && hidden !== 8) throw new Error('hidden width must be 4 or 8');
  if (!['net-only', 'residual-shipped-hce'].includes(mode)) throw new Error('invalid evaluator mode');
  const expected = expectedMetadata(hidden, mode);
  for (const [key, value] of Object.entries(expected)) {
    if (metadata[key] !== value) throw new Error(`invalid metadata ${key}`);
  }
  ...
  const numeratorBound = Q1 * w2.reduce((sum, value) => sum + abs(value), 0n) + abs(b2);
  if (numeratorBound * CP_SCALE + Q1 * Q2 / 2n > INT64_MAX) throw new Error('theoretical int64 output overflow');
  return {hidden, mode, w1, b1, w2, b2, accumulatorBounds, numeratorBound};
}
...
function infer(model, fen, baselineCp) {
  if (model.mode === 'residual-shipped-hce') {
    if (!Number.isSafeInteger(baselineCp) || Math.abs(baselineCp) > SCORE_LIMIT) {
      throw new Error('residual mode requires integer baselineCp within +/-10000');
    }
  } else if (baselineCp !== undefined) {
    throw new Error('net-only mode forbids baselineCp');
  }
  ...
  const numerator = ordered.reduce((sum, value, i) => sum + value * model.w2[i], model.b2);
  if (abs(numerator) > model.numeratorBound) throw new Error('declared output numerator bound exceeded');
  const denominator = Q1 * Q2;
  const stm = floorDivide(numerator * CP_SCALE + denominator / 2n, denominator);
  let cpWhite = stm * BigInt(position.turn);
  if (model.mode === 'residual-shipped-hce') cpWhite += BigInt(baselineCp);
  ...
  // reporting, including residuals; BigInt overflow bounds were checked above.
...
if (require.main === module) {
  if (process.argv.length !== 4) throw new Error('usage: h4-v2-reference.js MODEL.bin METADATA.json');
