...
(function (global) {
  ...
  function analysisCtx(quiesce, positions, nodeBudget) {
    // Deterministic coaching analysis stays full-draft: Play-only selective
    // search experiments must be opted out here as well as in the scan.
  ...
  function analyse(state, opts) {
    ...
    const scan = ChessAI.think(state, { maxDepth: maxDepth, nodeLimit: scanNodes,
      quiesce: quiesce, positions: positions, randomize: false, lmr: false });
