 ...
 * dialog, reload → undo → replayed finish, the boot reconcile below —
 * simply overwrites the one record.
 */
(function () {
  'use strict';
  ...
  // games would corrupt every later statistic).
  function record(state, settings, status, gameId) {
    if (!gameId || !state.history.length || !status.over) {
